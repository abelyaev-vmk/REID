...
def _get_image_blob(sample, target_size=600):
...
def to_json_format(detections, feats=None, object_class=None):
    ...
    for num, det in enumerate(detections):
        bbox = {'x': int(det[0]), 'y': int(det[1]),
                ...
                'feat': list(map(lambda f: f, feats)) if feats is not None else None}
        bboxes.append(bbox)
    return bboxes
...
def test_probe(weights_path, output_dir, test_prototxt='models/vgg16/test_query.prototxt', dataset_names=None):
    ...
    probs, blocks = list(map(np.array, [probs, blocks]))
    np.save('logs/last_run/probs.npy', probs)
    np.save('logs/last_run/blocks.npy', blocks)
    print(np.mean(np.array(list(map(lambda p: np.argsort(p)[::-1][0], np.array(probs).reshape(100, 5533)))).ravel()))
